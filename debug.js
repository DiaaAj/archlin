...
const fixRegistry = require('./registry');
...
program
  .name('deploy')
  .description('Deploy AWS CDK deployment with recursive debugging using Claude')
  .version('1.0.0')
    ...
    while (attempt < maxAttempts && !success) {
      ...
      if (lastError) {
        ...
        try {
          ...
          const { fixedFiles, summary, fixId } = result;
      ...
      try {
        ...
        if (lastFixId) {
          await fixRegistry.updateFix(lastFixId, {
            status: "successful"
          });
          console.log(chalk.blue(`Updated fix registry: Marked fix ${lastFixId} as successful`));
        }
        ...
        const isNewError = !isSameError(error, previousError);

        if (lastFixId && previousError && isNewError) {
          await fixRegistry.updateFix(lastFixId, {
            status: "successful",
          });
          console.log(chalk.blue(`Updated fix registry: Marked fix ${lastFixId} as successful`));
          ...
          await fixRegistry.updateFix(lastFixId, {
            status: "failed",
            failureReason: `Deployment failed with error: ${error.message}`,
          });
          console.log(chalk.blue(`Updated fix registry: Marked fix ${lastFixId} as failed`));
        }
        ...
        const errorLogDir = path.join(projectDir, '.deployment-error-logs');
        await fs.mkdir(errorLogDir, { recursive: true });
        const errorLogPath = path.join(errorLogDir, `deployment-error-${attempt}.log`);
...
async function fixDeploymentErrors(projectDir, plantUmlContent, error, fixHistory) {
  ...
  const components = fixRegistry.extractComponentsFromError(error);
  const errorPattern = fixRegistry.extractErrorPattern(error);
  console.log(chalk.blue(`Identified components: ${components.join(', ')}`));
  console.log(chalk.blue(`Extracted error pattern: ${errorPattern}`));
  
  const relevantFixes = await fixRegistry.getFixesForErrorPattern(errorText);
  console.log(chalk.blue(`Found ${relevantFixes.length} relevant fixes in registry`));

  let registryContext = '';
  if (relevantFixes.length > 0) {
    registryContext = '\nBased on our fix registry, here are some approaches that might be relevant:\n\n';
    
    const successfulFixes = relevantFixes.filter(fix => fix.status === 'successful');
    if (successfulFixes.length > 0) {
      registryContext += 'SUCCESSFUL APPROACHES:\n';
      successfulFixes.forEach(fix => {
        registryContext += `- For ${fix.components.join(', ')} with error "${fix.errorPattern}": ${fix.explanation}\n`;
      });
    }
    
    const failedFixes = relevantFixes.filter(fix => fix.status === 'failed');
    if (failedFixes.length > 0) {
      registryContext += '\nAPPROACHES THAT FAILED:\n';
      failedFixes.forEach(fix => {
        registryContext += `- DO NOT try: ${fix.attemptedSolution} - Reason: ${fix.failureReason}\n`;
      });
    }
  }
...
IMPORTANT: Format your response as a JSON object with the following structure:
{
  ...
  "explanation": "Detailed technical explanation of why the error occurred and how your fix addresses the root cause",
  "files": [
    {
...
The explanation should be detailed and focus on the root cause of the issue for future learning.
...
`;
  ...
  try {
    ...
    const {errorHash, errorCleantext} = fixRegistry.generateErrorHash(error); 

    const fixData = {
      status: "attempting", // We'll update this later
      components: components,
      errorPattern: errorPattern,
      errorHash: errorHash,
      errorCleantext: errorCleantext,
      explanation: result.explanation || result.summary,
      solutionTemplate: result.files.map(file => 
        `// ${file.filename}\n${file.content.substring(0, 200)}...`
      ).join('\n\n'),
      cdkVersions: [await fixRegistry.getCdkVersion(projectDir)],
      lastUpdated: new Date().toISOString()
    };
    
    lastFixId = await fixRegistry.addFix(fixData);
    console.log(chalk.blue(`Added fix to registry with ID: ${lastFixId}`));
...
function isSameError(error1, error2) {
  if (!error1 || !error2) return false;
  
  return fixRegistry.isSameErrorByHash(error1, error2);
}
