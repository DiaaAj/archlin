...
async function generateCdkCode(plantUmlContent, contextContent = '') {
  ...
  const components = await extractComponentsFromPlantUML(plantUmlContent);
  console.log(chalk.blue(`Identified AWS components in diagram: ${components.join(', ')}`));
  
  const relevantFixes = await Promise.all(components.map(comp => 
    fixRegistry.getFixesForComponents(comp)
  ));
  
  const flattenedFixes = Array.from(new Set(relevantFixes.flat().map(fix => fix.id)))
    .map(id => relevantFixes.flat().find(fix => fix.id === id));
  
  console.log(chalk.blue(`Found ${flattenedFixes.length} relevant fixes in registry`));
  
  let registryContext = '';
  if (flattenedFixes.length > 0) {
    registryContext = `\n\nIMPORTANT - Based on our experience, please incorporate these learnings when generating the CDK code:\n`;
    
    // Add successful fixes as best practices
    const successfulFixes = flattenedFixes.filter(fix => fix.status === 'successful');
    if (successfulFixes.length > 0) {
      registryContext += '\nBEST PRACTICES:\n';
      successfulFixes.forEach(fix => {
        registryContext += `- When working with ${fix.components.join(' and ')}: ${fix.explanation}\n`;
      });
    }
    
    // Add failed fixes as anti-patterns
    const failedFixes = flattenedFixes.filter(fix => fix.status === 'failed');
    if (failedFixes.length > 0) {
      registryContext += '\nCOMMON PITFALLS TO AVOID:\n';
      failedFixes.forEach(fix => {
        registryContext += `- DO NOT ${fix.attemptedSolution} when working with ${fix.components.join(' and ')}. Instead, ${fix.correctApproach || 'use proper IAM permissions and resource configurations'}.\n`;
      });
    }
  }
...
For package.json:
  - Include ALL necessary AWS CDK dependencies
  - Minimum required dependencies:
    * "aws-cdk-lib": "2.89.0" or newer
    * "constructs": "^10.0.0"
    * "source-map-support": "^0.5.21"
  - Depending on the components in the diagram, also include relevant AWS CDK specific packages

For tsconfig.json:
  - MUST include the "include" field with: ["bin/**/*", "lib/**/*"]
  - Include these exact settings:
    * "target": "ES2018"
    * "module": "commonjs"  
    * "lib": ["es2018"]
    * "declaration": true
...
async function extractComponentsFromPlantUML(plantUmlContent) {
  const components = new Set();
  
  // Map of common PlantUML elements to AWS components
  const componentMappings = {
    'Lambda': ['Lambda', 'AWS Lambda', 'lambda'],
    'S3': ['S3', 'S3 Bucket', 'Bucket', 'Storage'],
    'DynamoDB': ['DynamoDB', 'Dynamo', 'Table', 'NoSQL'],
    'API Gateway': ['API Gateway', 'APIGW', 'Gateway', 'REST API'],
    'CloudFront': ['CloudFront', 'CDN'],
    'EventBridge': ['EventBridge', 'Events', 'Event Bus'],
  ...
  // Look for components in the PlantUML content
  for (const [component, keywords] of Object.entries(componentMappings)) {
    for (const keyword of keywords) {
      if (plantUmlContent.includes(keyword)) {
